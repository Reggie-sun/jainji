...
自动制作将已核查的原贴纸事实与证据保存在本机应用用户目录；相同字节副本可跨项目复用，改名不影响，同名不同内容或重编码不可命中。已有核查时段必须覆盖本次请求：前 3 秒知识不能当作全程结果，全程知识可服务前 3 秒。新制作的贴纸均按全程识别：价格选择前 5 秒渐隐不会缩短贴纸时域，普通贴纸持续保留，覆盖与补角按有效时段显示。历史冻结模板重试仍保留旧时序。更换手动展示文字、款式或输出设置仍逐版本创作和检查样片；命中不免除视觉识别与复核连接配置。
...
作品列表显示首次检查、复用、修正、检查完成或未保存供复用等状态；“查看核查说明”展开本轮请求、修订、渲染次数、耗时和核查区间。识别次数包含执行识别和主管识别，样片次数指主管请求；这些是抽样核查与本轮计数，不是逐帧保证或商业费用。普通服务错误不会撤销已有可信知识，但当前失败版本不导出。详细合同见 [规格](docs/source-sticker-knowledge-spec.md)。

自动制作的终态统计由同一知识库保存，覆盖成功入队、失败与取消，最多保留最近 1000 条 / 2 MiB。计数是 provider 方法调用次数，不是计费凭据；模型结论不等于人工画面验收，终态保存前的硬崩溃可能没有记录。旧结果采用的知识后来被质疑时，作品列表显示只读警告，即使该知识已被新修订替代；知识库缺失则提示历史状态未知。这些提示不修改已冻结的模板，也不阻止按原方案重试。
