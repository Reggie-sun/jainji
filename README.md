...
3. 选择黑金精选、清爽日常或黑白叙事模板。中间只显示手动填写的产品价格（元），未填不显示，同一批视频使用同一价格；可设置制作倍数，例如 3 条素材 × 2 倍 = 6 条成片，每个版本独立调用 Agent 设计并导出（效果可能相似），项目最多保留 100 条导出记录；Agent 不得改写价格或在新增文案中加入产品名。可选填文案偏好等补充要求，并选择输出文件夹。
