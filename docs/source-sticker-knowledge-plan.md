...
# Automatic Cover Scope Update

后续自动覆盖改用独立的近似覆盖渲染方案，详见 [规格边界更新](source-sticker-knowledge-spec.md#automatic-cover-boundary-update)。本计划的知识持久化、证据与修订传播继续用于关闭覆盖的自动补角；近似位置不计为源知识验收，也不据此宣称 M5 整体验收完成。
