...
真实原片的一个连续静态短 segment 已能建立源空间保守候选，Codex 在全部 90 帧的联系图上检查了边缘；无贴纸、证据不足、变化图形和视觉证据反驳自动候选均有可重复的拒绝结果。**M1 尚未收敛到可进入 M2 的生产合同**：独立人工边缘核查仍未完成；第二个留出样本被移动爱心否决，小图案超出当前门槛；还缺独立的、确认为静态且可建立 mask 的留出样本，用来验证边缘保守性和冻结扩张的半径、面积及宽高上限。现有 1.8 秒星形通过率不能代替该留出验证。按 plan 保持正式白底路径及规则，继续 M1 样本采集；不得先实现或启用轮廓 renderer。
