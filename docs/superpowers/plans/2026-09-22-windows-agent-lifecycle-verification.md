...
**Goal:** 回写两份既有文档（`docs/windows-acceptance-spec.md` 增加 W21–W28 测试用例；`docs/batch-video-production-agent-playbook.md` 增加 Windows Adaptation 小节），把 Windows 上产品 Agent 全链路与外部 agent IPC 表面的验收契约落地。
...
- 既有 W01–W20 与 Linux 步骤一律不动；W21–W28 与 Windows Adaptation 是追加而非替换。
- W21–W28 Priority 列与既有 W 表保持一致（P0 / P1），不接受未标注的新等级。
...
- 提交：每个 Task 单独 commit；commit message 不携带 emoji；按既定规范加上 `Co-Authored-By: Claude Code <noreply@anthropic.com>` 末尾行。
- Dual-review gate 在最终 Task 启动；Codex native reviewer 与 Kimi deep reviewer 对同一 immutable snapshot 独立审查。

---

### Task 1: 回写 `docs/windows-acceptance-spec.md`：在 Test Cases 表追加 W21–W28
...
- Consumes: [`docs/superpowers/specs/2026-09-22-windows-agent-lifecycle-verification-design.md`](../specs/2026-09-22-windows-agent-lifecycle-verification-design.md) §Windows Test Matrix（W21–W28 八行 + 测试机制说明）
- Produces: 既有 markdown 表扩展 8 行（ID W21–W28），新增一列 `Stage`（与既有表四列并列）；表前说明段落保持不动。
...
W02 到 W20 共 19 行同样处理。逐行校验，避免一次 `replace_all` 误伤。完成后 `git diff docs/windows-acceptance-spec.md` 只显示列结构变化与 W21–W28 新行，不应改动其它字符。

- [ ] **Step 4: 在 W20 之后追加 W21–W28 行**

直接复制规格的 Windows Test Matrix 表（W21 至 W28 共 8 行）到 W20 行后：
...
| W22 | P1 | 2 | 在隔离 fixture 模型上 1 视频 1 版本手填价格；运行中尝试切换连接 | 模型抽 4 帧 → 本地中心 + 四角 → FFmpeg 渲染 → 输出验证通过；运行中切换连接被禁用；非法价格在 IPC 入口拒绝；非法方案显式失败且不重试不切换 |
| W23 | P1 | 4 | 覆盖关闭的自动补角；覆盖关闭开关在「作品」可见 | 每角归属明确（源贴纸角 vs 补齐角），补齐时段在导出前可被补帧/放大，每窗口执行 1 次、主管最多 3 次；超时显式失败，不静默重试 |
...
紧接 W28 行后增加一段（与规格中"测试机制"行内容一致）：

```markdown
**测试机制**：W21–W28 与既有 W01–W20 共享同一验证基础。`packaged-runtime-smoke`、`desktop-smoke`、`feedback-smoke` 复用；模型、登录、CC Switch、GitHub 继续用隔离 fixture，遵循 §Environment Record 的禁止项（不消耗真实账号、不提交真实 Issue）。
```
...
```markdown
- [Windows Agent Lifecycle Verification Spec](superpowers/specs/2026-09-22-windows-agent-lifecycle-verification-design.md) — W21–W28 设计契约
```
...
2. W01–W28 共 28 行全部存在；W21–W28 共 8 行的内容与上面粘贴的字符串逐字一致。
...
git commit -m "docs(windows-acceptance-spec): add W21-W28 Agent lifecycle test cases
...
- Consumes: [`docs/superpowers/specs/2026-09-22-windows-agent-lifecycle-verification-design.md`](../specs/2026-09-22-windows-agent-lifecycle-verification-design.md) §External Agent Windows Adaptation（WA.1–WA.6 完整内容）
...
- W21–W28 同样适用；任一 FAIL/BLOCKED/NOT_RUN 不能宣布「Windows Agent 验证完成」。
...
- Files changed (vs base before Task 1):
  - `docs/windows-acceptance-spec.md`（新增 Stage 列；新增 W21–W28；新增 References 一条）
  - `docs/batch-video-production-agent-playbook.md`（新增 `## Windows Adaptation` 及六个子节；新增 References 一条）
