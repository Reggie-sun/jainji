...
| 20 | PARTIAL | 一条真实素材冷、修复后暖、刷新出片，另有覆盖开启暖出片；缺旧基线、覆盖开启冷/刷新、完整场景矩阵及配对人工全片质量记录 |
...
## Automatic Cover Warm Pilot — 2026-09-18

用户明确要求开启覆盖。本轮在 `96dd8f1` fresh build 上沿用同源、模型、手动文字 `19.9元30贴`、`first-3s` 和导出配置，最多 10 次调用、服务失败即停。隔离目录 `/tmp/jianji-knowledge-cover-o2klUV` 从已关闭的刷新 pilot 复制完整知识，通过既有 `setCoverSticker` IPC 开启 `enabled=true / trackingMode=agent`，不提供历史人工坐标或替换模型结果，不修改原项目。

先前现场驱动在 `/tmp/jianji-knowledge-cover-jMiRbd` 漏传新项目尚未保存的 `stickerIds` / `rectangle` 默认字段，主进程拒绝设置；该次 runs=0、无模型请求。只修正一次性驱动，按界面的 `DEFAULT_COVER_STICKER` 补齐共享 schema 字段，再使用上述新目录开始模型运行；默认手动框被自动模式忽略，不是识别事实。保留失败预检记录，未改生产校验或重试失败模型。

Run `c3b9c0ec-b122-474b-ba40-780d42af87fd` 命中刷新修订 `c28dcc9b-d52e-4fca-8e92-6a35782c2e06`，识别执行/主管均 **0 次**；创作 **4 次**（覆盖初筛/选定及装饰初筛/设计）、样片主管 **1 次**，合计 **5 次**。新样片渲染 1 次、修订 0 次、主管 pass；约 103.3 秒入队，正式导出约 19.3 秒。暖复用没有跳过本版创作和样片检查。

冻结模板含唯一 `sparkle` 自动覆盖层：`opaqueBackground=true`，覆盖渲染轨迹 `[0,3000)`，扩边后左下框 `{x:0,y:0.87,width:0.19,height:0.13}`，不把渲染框当成源事实框；同角普通装饰的 activeRanges 从 3000 ms 才开始，且整版 `first-3s` 再限制所有新增层，因此不会在覆盖时段重复叠放，也不会在 3 秒后补出普通贴纸。模板引用同一知识修订/事实摘要，仍仅四个知识事件、65 条证据引用，无新发布或新增 dispute。

成片 `output/7a3cf3be8fb777e2760d8712a6050ace_edited.mp4` 位于上述隔离目录，10,439,102 字节，SHA-256 `e04b79237d2ae0e5318796450842059a272c93df2328a5edb7e79cc83c589ab9`。源 SHA、代码/build SHA、知识记录和全部证据摘要/字节数核验通过；视频/音频各自时长保持，完整严格解码通过。临时登录/API 配置副本已移除，owner 释放；报告、RPC 记录、验证脚本/结果与配对帧保留。

父 Agent 检查 1、2.733、3.1、20 秒成片并对照同源样本：1 秒原左下贴纸被白底覆盖层遮住，其余三角有普通贴纸；白色矩形明显可见，是当前不透明底板策略，不宣称自然无痕。2.733 秒覆盖层与文字、其他贴纸一起渐隐，原贴纸开始透出；3.1 和 20 秒新增层消失，原贴纸保留，符合前 3 秒设置，不代表全程覆盖。抽样未见同角重复新增贴纸；逐帧边界、全程漏盖/误盖、人工完整播放与音频听感仍未验收。

本轮无产品代码修改。fresh build（含 typecheck、3144 资产/许可证验证）通过；知识 integration 与 four-corner-coverage 两份 suites **20/20 通过**，现有 bundle 大小提示保留。该单样本证明关闭覆盖时积累的源事实可服务本次开启覆盖的暖制作；不替代覆盖开启冷/刷新或其他素材的真实验证，M5 / AC-20 仍为 PARTIAL。

native `reviewer_xhigh` 独立核对调用、冻结覆盖层、知识事件和全部证据、输出摘要，并重跑 ffprobe 与完整严格解码；现场证据 accept，文档复核无阻断项。按其建议区分源事实框与扩边后的覆盖渲染框；本次视觉抽样由父 Agent 完成，不宣称独立人工全片质量通过。

## Next Checkpoint

下一步补覆盖开启的冷/刷新、其余代表性素材和旧基线对照，先冻结范围、配置和调用预算；不因本次成功自动扩大付费运行。Windows 实机和人工完整播放仍未验证。
