...
desktopSmoke: try {
  ...
  assert.equal(await evaluate("document.querySelector('nav button.active')?.textContent.includes('上传贴纸')"), true, "opening the project list does not navigate away from the current page");
  ...
  await waitFor(smokeScope === "saved-project-open" ? "document.querySelector('#creative-brief') !== null && document.querySelector('.saved-projects-dialog') === null" : "document.body.innerText.includes('测试素材.mp4')");
  ...
  await evaluate("document.querySelector('[aria-label=\"打开项目\"]').click()");
  await waitFor("document.querySelector('.saved-projects-dialog[open]') !== null");
  await evaluate(`document.querySelector('[data-recent-id=${JSON.stringify(savedEntry.id)}]').click()`);
  await waitFor(`!document.querySelector('[data-recent-id=${JSON.stringify(savedEntry.id)}]').disabled`);
  assert.equal((await evaluate("window.jianji.getState()")).project.name, "尚未保存的新名称", "cancelled open preserves unsaved names");
  await evaluate("document.querySelector('[aria-label=\"关闭项目列表\"]').click()");
  await click("保存项目");
  await waitFor("document.body.innerText.includes('项目已保存')");
  ...
  await evaluate(`document.querySelector('[aria-label=${JSON.stringify(`重命名项目 ${managedEntry.name}`)}]').click()`);
  assert.equal((await evaluate("window.jianji.getState()")).project.id, managementProjectId, "renaming a saved collection does not load it");
  ...
  assert.equal(await evaluate("document.querySelectorAll('.saved-projects-row').length"), 1, "each saved collection has one visible action row");
  ...
  await waitFor("[...document.querySelectorAll('.saved-projects-action.delete')].some(button => button.getAttribute('aria-label') === '删除项目 重命名项目')");
  await screenshot("09-saved-collection-list-delete");
  await evaluate("[...document.querySelectorAll('.saved-projects-action.delete')].find(button => button.getAttribute('aria-label') === '删除项目 重命名项目').click()");
