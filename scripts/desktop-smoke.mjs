...
desktopSmoke: try {
  ...
  assert.equal(await evaluate(`document.querySelector('[aria-label="已保存的项目"] option[value=${JSON.stringify(savedEntry.id)}]') !== null`), true, "saved project is directly visible");
  if (smokeScope === "saved-project-open") await screenshot("saved-project-before-open");
  await evaluate(`document.querySelector('[aria-label="已保存的项目"]').value = ${JSON.stringify(savedEntry.id)}; document.querySelector('[aria-label="已保存的项目"]').dispatchEvent(new Event('change', { bubbles: true }))`);
  ...
  await evaluate(`document.querySelector('[aria-label="已保存的项目"]').value = ${JSON.stringify(savedEntry.id)}; document.querySelector('[aria-label="已保存的项目"]').dispatchEvent(new Event('change', { bubbles: true }))`);
  await waitFor("!document.querySelector('[aria-label=\"已保存的项目\"]').disabled");
  ...
  await evaluate(`document.querySelector('[aria-label="已保存的项目"]').value = ${JSON.stringify(managedEntry.id)}; document.querySelector('[aria-label="已保存的项目"]').dispatchEvent(new Event('change', { bubbles: true }))`);
  await waitFor("[...document.querySelectorAll('button')].some(button => button.textContent === '重命名' && !button.disabled)");
  await click("重命名");
  await evaluate("document.querySelector('[aria-label=\"新的项目名称\"]').select()");
  await send("Input.insertText", { text: "重命名项目" });
  ...
  assert.equal((await evaluate("window.jianji.getState()")).recentProjects[0].name, "重命名项目");
  await waitFor("[...document.querySelectorAll('.saved-collection-row button')].some(button => button.textContent === '删除' && !button.disabled)");
  ...
  assert.equal((await evaluate("window.jianji.getState()")).project.mediaItems.length, 0, "deleting the active saved project opens a new workspace");
  assert.equal(await readFile(collectionFile + ".trashed", "utf8").then(() => true, () => false), true, "project file is moved to trash");
