...
try {
  ...
  await evaluate(`document.querySelector('[aria-label=${JSON.stringify(`重命名素材集 ${managedEntry.name}`)}]').click()`);
  ...
  assert.equal(await evaluate("document.querySelectorAll('.saved-collection-option-row').length"), 1, "each saved collection has one visible action row");
