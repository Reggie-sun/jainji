...
it.for(["manual", "agent"] as const)("renders every %s price style with distinct pixels, preserving landscape and portrait video/audio", { timeout: 60_000 }, async (mode, context) => {
  ...
  for (const [width, height] of [[360, 640], [640, 360]]) {
    ...
    for (const style of PRICE_STYLES) {
      const template = materializePlan({ summary: "花字", captions: [], filter: "warm", intensity: 0.4, ...(mode === "agent" ? { stickers: automaticHeartStickers(), priceStyle: style.id } : {}) }, "black-gold", media, assets, { mode, productPrice: "19.9元30贴", ...(mode === "manual" ? { priceStyle: style.id } : {}), sticker: "none" }, mode === "agent" ? automaticCatalog : undefined);
