...
function sticker(overrides: Partial<StickerLayer> = {}): StickerLayer {
  return {
    ...
    x: 0.005, y: 0.005, width: 0.08, rotationDeg: 0, opacity: 1, zIndex: 2, visible: true,
    ...overrides,
  };
}
...
describe("rule-constrained layout agent", () => {
  ...
  it("snaps renderer edits to a valid corner and enforces total sticker coverage", () => {
    ...
    expect(snapped).toMatchObject({ x: 0.895, y: 0.88, width: 0.1, rotationDeg: 15 });
    const template = { ...createDefaultTemplate(), layoutPolicy: "corner-safe-v2" as const };
    expect(() => EditTemplateSchema.parse({ ...template, layers: [sticker({ width: 0.1 }), sticker({ x: 0.895, width: 0.1 })] })).not.toThrow();
  ...
  it("accumulates pointer movement from drag start so a sticker can switch corners", () => {
    ...
    expect(position).toEqual({ x: 0.555, y: 0.755 });
    expect(rendered).toMatchObject({ x: 0.915, y: 0.88 });
  });

  it("fits the rotated preview bounding box before anchoring it to a corner", () => {
    const geometry = constrainedStickerPreviewGeometry({
      layer: sticker({ x: 0.895, y: 0.88, width: 0.1, rotationDeg: 15 }),
    ...
    expect(geometry.x + geometry.width).toBeCloseTo(0.995, 10);
    expect(geometry.y + geometry.height).toBeCloseTo(0.995, 10);
