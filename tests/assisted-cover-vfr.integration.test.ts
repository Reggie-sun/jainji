...
it("keeps VFR frame clocks and excludes the gap between human visible segments", async () => {
  ...
  await run(["-f", "lavfi", "-i", "color=c=black:s=64x64:r=20:d=1", "-vf", "select='eq(n,0)+eq(n,2)+eq(n,4)+eq(n,7)+eq(n,10)+eq(n,14)+eq(n,19)'", "-vsync", "vfr", "-c:v", "libx264", sourcePath]);
  ...
  const compiled = await new TemplateCompiler().compile(template, media, { ...DEFAULT_PRESET, resolutionMode: "source" }, { ffmpegPath: "ffmpeg", fontResolver: { resolve: async () => null }, textFilePath: (id) => path.join(root, `${id}.txt`), threads: 1 });
  await Promise.all(compiled.textFiles.map((file) => writeFile(file.path, file.content)));
  await run(compiled.args.concat(outputPath));
  const times = async (file: string) => { const result = await runCommand("ffprobe", ["-v", "error", "-select_streams", "v:0", "-show_entries", "frame=best_effort_timestamp_time", "-of", "json", file]).promise; return JSON.parse(result.stdout).frames.map((frame: { best_effort_timestamp_time: string }) => Number(frame.best_effort_timestamp_time)); };
  const sourceTimes = await times(sourcePath), outputTimes = await times(outputPath);
  expect(outputTimes).toEqual(sourceTimes);
  const pixels = path.join(root, "output.rgb"); await run(["-i", outputPath, "-vsync", "0", "-pix_fmt", "rgb24", "-f", "rawvideo", pixels]);
