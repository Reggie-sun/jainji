...
it.for(["manual", "agent"] as const)("renders %s stickers and only a local center price with real FFmpeg", { timeout: 60_000 }, async (mode, context) => {
  ...
  const template = mode === "manual"
    ...
    : materializePlan({ summary: "按需装饰", captions: [], priceStyle: "classic", stickers: [{ corner: "bottom-right", sticker: "heart" }], filter: "warm", intensity: 0.4 }, "black-gold", { width: 720, height: 1280 }, assets, DecorationSchema.parse({ mode: "agent", productPrice: "19.90" }), { fonts: [DEFAULT_TEXT_FONT_FAMILY], stickers: [{ id: "heart", label: "爱心" }] });
