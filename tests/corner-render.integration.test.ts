...
it.for(["manual", "agent-mixed", "agent-empty"] as const)("renders %s corner choices with real FFmpeg", { timeout: 60_000 }, async (mode, context) => {
  ...
  const manualTemplate = materializePlan({ summary: "fixture", captions: [{ text: "自动文案", size: 0.026, corner: "top-left" }], filter: "warm", intensity: 0.4 }, "black-gold", { width: 720, height: 1280 }, assets, DecorationSchema.parse({ corners: {
  ...
  }, "black-gold", { width: 720, height: 1280 }, assets, DecorationSchema.parse({ mode: "agent" }), { fonts: [DEFAULT_TEXT_FONT_FAMILY], stickers: [{ id: "heart", label: "爱心" }] });
  expect(template.layers).toHaveLength(mode === "manual" ? 4 : mode === "agent-empty" ? 0 : 2);
  ...
  expect(compiled.textFiles.map((entry) => entry.content).sort()).toEqual((mode === "manual" ? ["右下精选", "左上好物"] : mode === "agent-empty" ? [] : ["自动好物"]).sort());
