...
it("preserves original corner stickers and renders only gap decorations with audio", { timeout: 60_000 }, async context => {
  ...
  const root = await mkdtemp(path.join(tmpdir(), "jianji-source-corner-render-"));
  const sourcePath = path.join(root, "source.mp4"), normalPath = path.join(root, "normal.png");
  ...
  const rectangle = { x: 0, y: 0, width: 0.04, height: 0.04 };
  ...
  expect(frozen.layers.some(layer => layer.type === "sticker" && layer.cover)).toBe(false);
  ...
  const original = await pixels(4, 4, 4); expect(original[0]).toBeGreaterThan(220); expect(original[1]).toBeGreaterThan(220); expect(original[2]).toBeLessThan(30);
  ...
  console.info(`source-corner preservation evidence: ${output}`);
});
