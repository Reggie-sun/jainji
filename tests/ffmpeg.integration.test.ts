...
describe("real FFmpeg proof render", () => {
  ...
  it("renders Unicode text, transparent sticker and filter, then verifies the artifact", async (context) => {
    ...
    template.layers.push({ id: crypto.randomUUID(), type: "sticker", assetPath: stickerPath, assetFingerprint: "fixture", x: 0.895, y: 0.88, width: 0.1, rotationDeg: 7, opacity: 0.8, zIndex: 3, visible: true });
  ...
  it("preserves motion from a bundled GIF sticker", async (context) => {
    ...
    const source = await runCommand(ffmpegPath, ["-hide_banner", "-y", "-f", "lavfi", "-i", "color=c=0x224466:s=320x180:r=25:d=6", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-an", sourcePath]).promise;
    expect(source.code, source.stderr).toBe(0);
    const media: MediaItem = { id: crypto.randomUUID(), sourcePath, displayName: "source.mp4", fingerprint: "fixture", sizeBytes: 1, durationMs: 6_000, width: 320, height: 180, rotation: 0, probeStatus: "ready", importedAt: new Date().toISOString() };
    ...
    const hashes = await runCommand(ffmpegPath, ["-v", "error", "-ss", "4", "-i", outputPath, "-vf", "fps=10", "-t", "2", "-f", "framemd5", "-"]).promise;
