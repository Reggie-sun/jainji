...
describe("local price styles", () => {
  ...
  it.each(PRICE_STYLES)("freezes $id into exactly one local price layer and compiles its effects", async (entry) => {
    ...
    const auto = materializePlan({ ...plan, stickers: [], priceStyle: style.id }, "black-gold", media, assets, { mode: "agent", productPrice: "19.9元30贴", priceStyle: style.id }, { fonts: [], stickers: [] });
