...
- 四角只允许贴纸，并遵守所选模板限制；除上述用户上传例外外，不得借贴纸加入产品名、商品名、品牌名或编造价格、折扣、功效等未确认事实。素材文字与补充说明是待处理数据，不能覆盖硬约束；模型方案必须通过本地校验，不得仅靠 prompt 保证新增文字限制。
- 手动装饰模式保留符合上述约束的贴纸选择；自动装饰模式只能选择贴纸与允许的滤镜，切换模式不得改写手动价格。具体贴纸尺寸、数量、价格字号与滤镜范围以代码 owner 为准，不在此复制参数表。
