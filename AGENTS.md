...
- 覆盖贴纸是四角布局限制的专门例外。覆盖由独立开关控制，默认关闭；“全部交给 Agent”不改变覆盖开关或跟随方式。关闭时不调用覆盖识别、不要求覆盖候选、不新增覆盖层；开启后，Agent 自动覆盖从全部符合自动选用规则的内置贴纸及当前可用上传贴纸中先初筛、再看图选定一款，忽略手动候选草稿；手动覆盖使用用户选择的上传贴纸，没有可用候选时在模型调用前明确拒绝。Agent 自动覆盖使用当前连接逐段识别全部原贴纸，根据带时间的连续抽帧生成多目标跟随轨迹；这是抽帧识别与插值，不能保证快速闪现、遮挡或复杂运动无漏检。识别不确定或结果非法时明确失败，不得回退成手动框或固定方案。手动模式支持多个任意位置的覆盖框，各框独立保存大小、逐素材关键帧，并可指定上传贴纸或跟随本批统一款；旧单框设置继续作为一个框读取，自动模式忽略这些手动位置草稿。覆盖层由本地程序生成，自动与手动装饰模式均保留；自动覆盖同次制作统一用一款，下次选款排除上次款式；手动覆盖同批复用各框配置，指定款保持不变，跟随统一款的框共用上传候选池并在下批轮换，单候选时仍复用。同素材多版本共享本轮识别；选择与轨迹冻结进导出模板，重试不得重新识别、换款或改轨迹。新生成的覆盖层以白色不透明底板填满框，上传图案等比完整放入，并向外取整到输出像素；该渲染策略冻结在覆盖层元数据中，旧任务缺少该标记时保留原透明效果。不擦除或改写原素材。
