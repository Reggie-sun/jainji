...
- 保留原视频的顺序、时长、帧率与音频。新任务默认按横竖方向导出 720p，等比缩放并补边，不裁剪画面；显式原分辨率设置与历史任务冻结设置继续保留。未经用户明确扩展范围，不加入裁剪、拼接、语音转写、配音或新素材生成。
...
- 自动模式的模板只描述允许的能力与安全边界，不得将风格预设或轮换构图作为固定方案。Agent 按画面自主选择贴纸、角落、尺寸、旋转及支持的滤镜，也可留空或保留原色；手动模式保留用户选择和风格预设。切换模式不得改写手动价格。具体贴纸尺寸、数量、价格字号与滤镜范围以代码 owner 为准，不在此复制参数表。
