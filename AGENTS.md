...
- 用户启用的覆盖贴纸是四角布局限制的专门例外：覆盖框可位于原素材任意区域，位置、大小和用户上传的候选池由用户设置并随素材集保存。每条素材可设置手动关键帧轨迹与出现时段，关键帧之间线性移动、等比缩放；无轨迹的素材继续使用共用固定框。覆盖层由本地程序生成，自动与手动模式均保留；同次制作统一用一款，下次从候选池轮换，单候选时仍复用。选择、区域与轨迹冻结进导出模板，重试不得换款或改轨迹。覆盖图等比填满框并裁切，透明处仍可能露出原画面；不擦除原素材、不自动识别或追踪移动贴纸。
