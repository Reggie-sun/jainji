...
- 用户可选择后期展示文字 / 价格全程显示或仅前 3 秒显示；默认全程。新制作的普通贴纸全程保留，覆盖贴纸与四角补齐按全程识别的有效时段显示，不随价格渐隐。前 3 秒模式在最后 0.5 秒渐隐，短视频在结尾前渐隐，不改变原视频内容、时长或音频。该选项与贴纸全程标记告知 Agent 并冻结进模板，由本地渲染执行；缺少贴纸全程标记的历史模板保持旧的联合时序，重试沿用。
