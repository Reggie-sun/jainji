...
- 覆盖贴纸是四角布局限制的专门例外。“全部交给 Agent”自动启用原贴纸识别与覆盖，不受手动覆盖开关或轨迹草稿影响；优先使用已保存的覆盖候选，未选候选时使用当前可用的用户上传贴纸，没有可用候选时在模型调用前明确拒绝。手动装饰模式仍使用独立覆盖开关。Agent 自动覆盖使用当前连接逐段识别全部原贴纸，根据带时间的连续抽帧生成多目标跟随轨迹；这是抽帧识别与插值，不能保证快速闪现、遮挡或复杂运动无漏检。识别不确定或结果非法时明确失败，不得回退成手动框或固定方案。手动模式保留任意位置的固定框与逐素材关键帧设置，自动模式忽略这些手动位置草稿。覆盖层由本地程序生成，自动与手动装饰模式均保留；同次制作统一用一款，下次从用户上传的候选池轮换，单候选时仍复用。同素材多版本共享本轮识别；选择与轨迹冻结进导出模板，重试不得重新识别、换款或改轨迹。覆盖图填满框并裁切，透明处仍可能露出原画面，不擦除原素材。
