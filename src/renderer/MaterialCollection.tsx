...
export function MaterialCollection({ name, dirty, disabled, openingDisabled, onSave, onOpen, onName, onRename, onDelete, recentProjects, activeRecentId }: {
  ...
  return <section className="card material-collection" aria-label="保存与打开素材集">
    ...
    <div className="saved-collections"><label htmlFor="saved-collection-select">已保存的素材集</label><div className="saved-collection-row"><select id="saved-collection-select" aria-label="已保存的素材集" value={activeRecentId ?? ""} disabled={openingDisabled || !recentProjects.length} onChange={(event) => {
