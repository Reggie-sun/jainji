  ...
  return <section className="price-style-picker" aria-label="价格花字样式">
    ...
    <div className="price-style-grid" role="group" aria-label="选择价格花字">
      {PRICE_STYLES.map((entry) => {
        ...
        return <button key={style.id} type="button" className="price-style-option" disabled={disabled} aria-pressed={selected.id === style.id} aria-label={`价格花字：${style.name}`} onClick={() => onChange?.(style.id as PriceStyleId)}>
    ...
    <small>仅改变手动价格的外观，切换模板或装饰模式会保留选择。未填价格时只展示字样示例。</small>
  </section>;
}
