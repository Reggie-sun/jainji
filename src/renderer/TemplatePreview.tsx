...
export function TemplatePreview({ rule, options, selectedCorner, onCornerSelect, disabled }: { rule: RuleTemplate; options: DecorationOptions; selectedCorner?: Corner; onCornerSelect?(corner: Corner): void; disabled?: boolean }) {
  ...
  return <section className="template-preview card" aria-label="整体模板预览">
    ...
    <div className="template-preview-info"><span className="eyebrow">{automatic ? "AGENT CAPABILITY" : "TEMPLATE PREVIEW"}</span><h2>{automatic ? "Agent 自主安排" : `${rule.name} · 整体预览`}</h2>{automatic ? <p>开始出片后，Agent 为四角准备候补贴纸。关闭覆盖时保留原贴纸，只补空缺角落和时段；开启覆盖时优先显示覆盖层，其余时段补齐。具体选款和时段将在识别后确定。</p> : <><p>先看一眼贴纸放在一起的效果，再开始制作。</p><dl><div><dt>贴纸</dt><dd>{stickerId === "none" ? "不加贴纸" : `宽度不超过画面的 ${(previewStickerWidth * 100).toFixed(0)}%`}</dd></div></dl></>}
