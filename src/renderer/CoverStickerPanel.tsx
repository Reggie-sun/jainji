...
export function CoverStickerPanel({ projectId, value, selectedMedia, revision, disabled, onSave, onDirtyChange }: {
  ...
  return <>
    ...
    <section className="card cover-sticker-panel" aria-label="覆盖原贴纸设置">
      ...
      {draft.enabled && <>
        <div className="cover-tracking-tabs" role="group" aria-label="覆盖贴纸跟随方式"><button type="button" aria-pressed={trackingMode === "agent"} disabled={disabled || saving} onClick={() => setDraft((current) => ({ ...current, trackingMode: "agent" }))}>Agent 看图自动覆盖</button><button type="button" aria-pressed={trackingMode === "manual"} disabled={disabled || saving} onClick={() => setDraft((current) => ({ ...current, trackingMode: "manual" }))}>手动设置</button><button type="button" aria-pressed={trackingMode === "assisted"} disabled={disabled || saving} onClick={() => setDraft((current) => ({ ...current, trackingMode: "assisted" }))}>半自动 · 人工审阅</button><button type="button" aria-pressed={trackingMode === "random"} disabled={disabled || saving} onClick={() => setDraft((current) => ({ ...current, trackingMode: "random" }))}>本地随机</button></div>
        {trackingMode === "assisted" ? <p>半自动覆盖由你编辑完整边界与时段，查看各版本动态预览后再确认导出。</p> : trackingMode === "agent" ? <p className="cover-agent-status">Agent 将从全部本地内置贴纸和可用上传贴纸中看图选款，无需逐张勾选；覆盖可原样使用贴纸自带的文字、价格或品牌图案，普通四角装饰规则不变。同轮素材统一用一款，下一轮换款；一次制作多轮也会轮换。</p> : trackingMode === "random" ? <p>本地随机覆盖从已上传贴纸中本地抽取，不调用任何模型。</p> : <>
