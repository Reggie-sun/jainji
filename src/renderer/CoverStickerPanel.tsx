...
export function CoverStickerPanel({ projectId, value, selectedMedia, revision, disabled, onSave, onDirtyChange }: {
  ...
  const needsSharedSticker = effectiveRegions.some((region) => !region.stickerId);
  const missingSticker = stickers !== undefined && [...draft.stickerIds, ...assignedStickerIds].some((id) => !stickers.some((sticker) => sticker.id === id));
  ...
  const save = async () => {
    ...
    if (draft.enabled && trackingMode === "manual" && needsSharedSticker && !draft.stickerIds.length) { setError("仍有覆盖框使用统一款，请至少选择一张自己的贴纸，或为每个覆盖框单独选择贴纸。"); return; }
    if (draft.enabled && trackingMode === "manual" && missingSticker) { setError("有已选或已分配贴纸不在当前上传素材库中，请重新选择后保存。"); return; }
    setSaving(true);
    try {
      await onSave(draft);
  ...
  const clearMissingStickers = () => setDraft((current) => {
    ...
    return { ...current, stickerIds: current.stickerIds.filter((id) => stickers?.some((sticker) => sticker.id === id)), regions: current.regions?.map(keep), mediaRegions: current.mediaRegions && Object.fromEntries(Object.entries(current.mediaRegions).map(([mediaId, mediaRegions]) => [mediaId, mediaRegions.map(keep)])) };
  });
  return <>
    ...
    <section className="card cover-sticker-panel" aria-label="覆盖原贴纸设置">
      <div className="cover-sticker-heading"><div><h2>{draft.enabled ? "覆盖已开启" : "覆盖已关闭"}</h2><p>{draft.enabled ? "选择覆盖贴纸与跟随方式；新覆盖层铺白色不透明底板并等比保留完整图案，保存后应用到下次制作。旧导出任务保留原效果。" : "关闭时保留原贴纸，不添加覆盖层。全部交给 Agent 时仍会识别原贴纸占位，只补空缺角落和时段。"}</p></div><label className="cover-sticker-toggle"><input type="checkbox" checked={draft.enabled} disabled={disabled || saving} onChange={(event) => setDraft((current) => ({ ...current, enabled: event.target.checked }))} />启用覆盖</label></div>
      {draft.enabled && <>
        <div className="cover-tracking-tabs" role="group" aria-label="覆盖贴纸跟随方式"><button type="button" aria-pressed={trackingMode === "agent"} disabled={disabled || saving} onClick={() => setDraft((current) => ({ ...current, trackingMode: "agent" }))}>Agent 自动识别全部原贴纸</button><button type="button" aria-pressed={trackingMode === "manual"} disabled={disabled || saving} onClick={() => setDraft((current) => ({ ...current, trackingMode: "manual" }))}>手动设置</button><button type="button" aria-pressed={trackingMode === "assisted"} disabled={disabled || saving} onClick={() => setDraft((current) => ({ ...current, trackingMode: "assisted" }))}>半自动 · 人工审阅</button></div>
        {trackingMode === "assisted" ? <p>半自动覆盖由你编辑完整边界与时段，查看各版本动态预览后再确认导出。</p> : trackingMode === "agent" ? <p className="cover-agent-status">Agent 将从全部可自动选用的内置贴纸和上传贴纸中看图选款，无需逐张勾选。同轮素材统一用一款，下一轮换款；一次制作多轮也会轮换。</p> : <>
          ...
          {missingSticker && <p className="cover-sticker-warning" role="alert">有已选或已分配贴纸不在当前上传素材库中，请重新选择。<button type="button" disabled={disabled || saving} onClick={clearMissingStickers}>清除失效贴纸</button></p>}
