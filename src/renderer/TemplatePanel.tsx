...
import { DecorationSchema, RequiredProductPriceSchema, type DecorationOptions, type Corner } from "../shared/decorations";
...
export function TemplatePanel({ selected, onSelect, brief, onBrief, outputDirectory, onOutput, onStart, count, disabled, decorations, decorationOptions, exportFormat, onExportFormat, selectedCorner, onCornerSelect, onGenerateBrief, generatingBrief = false, onProductPrice, requestedCount = count, onRequestedCount }: {
  ...
  return <>
    ...
    <div className="card brief-card"><label htmlFor="product-price">产品价格（元） <span>必填 · 手动输入</span></label><input id="product-price" required aria-invalid={invalidPrice} aria-describedby="product-price-help" inputMode="decimal" value={decorationOptions?.productPrice ?? ""} maxLength={9} disabled={disabled} onChange={(event) => onProductPrice?.(event.target.value)} placeholder="例如：19.90" /><small id="product-price-help">请手动填写产品价格，填写后才能开始制作。不能填写产品名，Agent 不能代填或改写价格。同一批视频使用此价格。</small>{invalidPrice && <p role="alert">请手动填写产品价格，最多两位小数，不要包含产品名或其他文字。</p>}</div>
    {decorations}
