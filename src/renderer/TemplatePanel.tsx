...
export function TemplatePanel({ selected, onSelect, brief, onBrief, outputDirectory, automaticOutput = false, onAutomaticOutput, onOutput, onStart, count, disabled, decorations, coverPanel, coverDirty = false, decorationOptions, exportFormat, onExportFormat, exportSettings = DEFAULT_EXPORT_SETTINGS, onExportSettings, selectedCorner, onCornerSelect, onGenerateBrief, generatingBrief = false, onProductPrice, onPriceStyle, onDisplayMode, requestedCount = count, onRequestedCount }: {
  ...
  return <>
    ...
    <div className="card brief-card"><label htmlFor="decoration-display-mode">价格 / 展示文字显示时段</label><select id="decoration-display-mode" value={decorationOptions?.displayMode ?? "full"} disabled={disabled || !onDisplayMode} onChange={(event) => onDisplayMode?.(event.target.value as DecorationDisplayMode)}><option value="full">全程显示（默认）</option><option value="first-3s">仅前 3 秒显示（渐隐）</option></select><small>仅控制后期添加的展示文字 / 价格；贴纸全程保留，覆盖与补角仍按各自有效时段显示，原视频内容不变。选项会告知 Agent；前 3 秒模式在最后 0.5 秒渐隐，短视频在结尾前渐隐。</small></div>
    {decorations}
