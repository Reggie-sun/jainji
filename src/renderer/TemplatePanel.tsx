...
export function TemplatePanel({ selected, onSelect, brief, onBrief, outputDirectory, onOutput, onStart, count, disabled, decorations, decorationOptions, exportFormat, onExportFormat, selectedCorner, onCornerSelect, onGenerateBrief, generatingBrief = false, onProductPrice, onPriceStyle, requestedCount = count, onRequestedCount }: {
  ...
  return <>
    <Heading eyebrow="02 / CHOOSE YOUR DIRECTION" title="定下风格，放手让它创作">模板规定边界。四角贴纸的位置与滤镜细节，由 Agent 根据每条素材决定。</Heading>
    ...
    <div className="card brief-card price-input-card"><label htmlFor="product-price">产品价格 / 数量 <span>必填 · 手动输入</span></label><textarea id="product-price" rows={2} required aria-invalid={invalidPrice} aria-describedby="product-price-help" inputMode="text" value={decorationOptions?.productPrice ?? ""} maxLength={PRODUCT_PRICE_MAX_LENGTH} disabled={disabled} onChange={(event) => onProductPrice?.(event.target.value)} placeholder={"9.9元到手5卷\n19.9元拍一发三"} /><small id="product-price-help">按 Enter 换行，最多2行、每行12字；可填写到手数量或拍一发三，填写后才能开始制作。不能填写产品名，Agent 不能代填或改写价格。同一批视频使用此价格。</small>{invalidPrice && <p role="alert">{PRODUCT_PRICE_HELP}</p>}<PriceStylePicker value={decorationOptions?.priceStyle} price={decorationOptions?.productPrice} disabled={disabled} onChange={onPriceStyle} /></div>
    {decorations}
    <div className="rules-banner"><div className="icon-tile"><Icon name="shield" /></div><div><strong>{rule.name} · 已锁定的创作边界</strong><p>贴纸仅放四角且宽度 ≤ {(rule.stickerWidth * 100).toFixed(0)}% · 新增文字仅限手动价格 · 不裁剪、不拼接 · 保留原始音频</p></div><span className="small-tag">本地校验</span></div>
