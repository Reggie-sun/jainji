import { RULE_TEMPLATES, getRule, MAX_AGENT_OUTPUTS, ProductionMultiplierSchema, type RuleId } from "../shared/agent";
...
export function TemplatePanel({ selected, onSelect, brief, onBrief, outputDirectory, onOutput, onStart, count, disabled, decorations, decorationOptions, exportFormat, onExportFormat, selectedCorner, onCornerSelect, onGenerateBrief, generatingBrief = false, onProductPrice, multiplier = 1, onMultiplier }: {
  multiplier?: number; onMultiplier?(value: number): void;
  ...
  const invalidQuantity = !ProductionMultiplierSchema.safeParse(multiplier).success || count * multiplier > MAX_AGENT_OUTPUTS;
  const total = Number.isFinite(multiplier) ? count * multiplier : 0;
  return <>
    ...
    <div className="card brief-card"><label htmlFor="production-multiplier">制作倍数</label><input id="production-multiplier" type="number" min={1} max={MAX_AGENT_OUTPUTS} step={1} value={Number.isFinite(multiplier) ? multiplier : ""} disabled={disabled} onChange={(event) => onMultiplier?.(event.target.valueAsNumber)} /><div role="group" aria-label="快捷制作倍数">{[1, 2, 3, 5, 10].map((value) => <button key={value} type="button" className="button secondary compact" aria-pressed={multiplier === value} disabled={disabled || count * value > MAX_AGENT_OUTPUTS} onClick={() => onMultiplier?.(value)}>{value} 倍</button>)}</div><p>{count} 条原素材 × {Number.isFinite(multiplier) ? multiplier : "—"} 倍 = {total} 条成片</p><small>每个版本独立设计并导出，价格和手动装饰保持一致；模型调用次数随成片数量增加。默认 1 倍。</small>{invalidQuantity && <p role="alert">倍数须为正整数，本轮最多制作 {MAX_AGENT_OUTPUTS} 条成片。</p>}</div>
