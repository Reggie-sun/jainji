...
export function ResultsPanel({ state, busy, retryingIds, onCancel, onRetry, onOpen, onReveal, onNew }: {
  ...
  const processing = tasks.filter((task) => !terminal.has(task.status)).length + unqueued.filter((item) => item.status === "waiting" || item.status === "analyzing" || item.status === "prepared").length;
