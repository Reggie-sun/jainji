  ...
  return <div className="app-shell">
    ...
    <div className="main-area">
      ...
      <main className="content">
        ...
        {step === "connection" && <ConnectionPanel key={state.connection.baseUrl + state.connection.model + state.connection.configured} connection={state.connection} chatgpt={state.chatgpt} busy={locked} onLogin={() => void run(async () => { apply(await window.jianji.loginChatGPT()); })} onCancelLogin={() => void run(async () => { apply(await window.jianji.cancelChatGPTLogin()); })} onUseCCSwitch={(id, appType) => run(async () => { apply(await window.jianji.useCCSwitch(id, appType)); })} onSave={(input) => run(async () => { apply(await window.jianji.configureAgent(input)); })} onTest={() => void run(async () => { await window.jianji.testAgent(); }, "文本连接测试通过。图片能力会在处理素材时验证。")} onDisconnect={() => void run(async () => { apply(await window.jianji.disconnectAgent()); })} onContinue={() => setStep("import")} />}
        {step === "import" && <>
