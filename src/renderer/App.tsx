...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <div className="main-area">
      <header className="topbar"><div className="breadcrumb">创作空间 <span>/</span> <strong>{step === "connection" ? "模型连接" : steps.find((item) => item.id === step)?.label}</strong></div><div className="topbar-actions"><span className={state.capabilities.ready ? "engine-status" : "engine-status unavailable"}><i />{engineLabel}</span><button className="icon-button" aria-label="查看素材" title="查看素材" disabled={locked || exporting} onClick={() => { setStep("import"); window.scrollTo({ top: 0, behavior: "smooth" }); }}><Icon name="folder" size={18} /></button><button className="button secondary compact" aria-label="打开项目" disabled={locked || exporting} onClick={() => setSavedProjectsOpen(true)}>打开项目</button><button className="button secondary compact" disabled={locked || !MaterialNameSchema.safeParse(collectionName).success} onClick={saveCollection}><Icon name="download" size={15} />保存项目</button></div></header>
