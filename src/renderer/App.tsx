...
import { CORNER_SAFE_POLICY, beginLayerDrag, constrainedStickerPreviewGeometry, cumulativeDragPosition, snapStickerToCorner, type LayerDragSession } from "../shared/layout-policy";
...
export default function App() {
  ...
  const updateTemplate = useCallback(async (next: EditTemplate) => {
    ...
    try { applyState(await window.jianji.updateTemplate(next)); }
      ...
      try { applyState(await window.jianji.getState()); } catch { /* keep the last visible state if refresh also fails */ }
      setNotice({ tone: "error", text: error instanceof Error ? error.message : "模板参数无效" });
    }
  }, [applyState, state]);
  ...
  const addSticker = async () => {
    ...
    if (!asset) return;
    const next = cloneTemplate(state.project.template);
    const rawLayer: Layer = { id: crypto.randomUUID(), type: "sticker", ...asset, x: 0.08, y: 0.08, width: state.project.template.layoutPolicy ? 0.16 : 0.25, rotationDeg: 0, opacity: 1, zIndex: next.layers.length, visible: true };
    const layer = state.project.template.layoutPolicy && rawLayer.type === "sticker" ? snapStickerToCorner(rawLayer) : rawLayer;
    next.layers.push(layer); setSelectedLayerId(layer.id); void updateTemplate(next);
  };
  ...
  const patchLayer = (layerId: string, patch: Partial<Layer>) => {
    ...
    if (index < 0) return;
    const patched = { ...next.layers[index], ...patch } as Layer;
    next.layers[index] = state.project.template.layoutPolicy && patched.type === "sticker" ? snapStickerToCorner(patched) : patched;
  ...
  const applyLayoutAgent = async (input: LayoutAgentInput) => {
    setBusy(true);
    try {
      applyState(await window.jianji.applyLayoutAgent(input));
      setSelectedLayerId(undefined);
      setNotice({ tone: "success", text: "规则出片 Agent 已生成小角标模板；贴纸角落与面积约束已锁定。" });
    } catch (error) { setNotice({ tone: "error", text: error instanceof Error ? error.message : "Agent 无法生成模板" }); }
    finally { setBusy(false); }
  };
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <section className="content">
        ...
        {step === "template" && <TemplateStep state={state} activeMedia={activeMedia} selectedLayer={selectedLayer} previewMediaId={previewMediaId} selectedLayerId={selectedLayerId} onPreviewMedia={setPreviewMediaId} onSelectLayer={setSelectedLayerId} onPatchLayer={patchLayer} onAddText={addText} onAddSticker={() => void addSticker()} onRemoveLayer={removeLayer} onMoveLayer={moveLayer} onUpdateFilter={(filter) => void updateTemplate({ ...cloneTemplate(state.project.template), filter })} onApplyAgent={(input) => void applyLayoutAgent(input)} onProof={() => void renderProof()} onSave={saveTemplate} onLoad={loadTemplate} onNext={() => setStep("export")} />}
...
function TemplateStep({ state, activeMedia, selectedLayer, previewMediaId, selectedLayerId, onPreviewMedia, onSelectLayer, onPatchLayer, onAddText, onAddSticker, onRemoveLayer, onMoveLayer, onUpdateFilter, onApplyAgent, onProof, onSave, onLoad, onNext }: { state: PublicState; activeMedia?: MediaView; selectedLayer?: Layer; previewMediaId?: string; selectedLayerId?: string; onPreviewMedia: (id: string) => void; onSelectLayer: (id: string) => void; onPatchLayer: (id: string, patch: Partial<Layer>) => void; onAddText: () => void; onAddSticker: () => void; onRemoveLayer: (id: string) => void; onMoveLayer: (id: string, direction: -1 | 1) => void; onUpdateFilter: (filter: EditTemplate["filter"]) => void; onApplyAgent: (input: LayoutAgentInput) => void; onProof: () => void; onSave: () => void; onLoad: () => void; onNext: () => void }) {
  const [agentTitle, setAgentTitle] = useState("今日好物推荐");
  const [agentPrice, setAgentPrice] = useState("19.9元2单");
  const [agentStyle, setAgentStyle] = useState<LayoutAgentInput["style"]>("black-gold");
  return <div className="step-panel"><SectionHeading kicker="02 / EDIT TEMPLATE" title="一次编辑，整批复用" subtitle="拖动预览中的图层，或用右侧数值精确调整。坐标始终基于自动旋转后的画面。" />
    <div className="editor-layout"><div className="editor-main"><div className="preview-toolbar"><label>代表性素材<select value={previewMediaId ?? ""} onChange={(event) => onPreviewMedia(event.target.value)}>{state.project.mediaItems.filter((item) => item.probeStatus === "ready").map((item) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</select></label><div className="toolbar-actions"><button className="ghost-button" onClick={onLoad}>加载模板</button><button className="ghost-button" onClick={onSave}>保存模板</button><span className="approx-badge">◌ 近似预览</span></div></div><Preview activeMedia={activeMedia} template={state.project.template} selectedLayerId={selectedLayerId} onSelectLayer={onSelectLayer} onMoveLayer={(id, patch) => onPatchLayer(id, patch)} /></div>
      <aside className="inspector"><div className="inspector-section agent-section"><div className="inspector-title"><span>规则出片 Agent</span><span className={`agent-mode ${state.project.template.layoutPolicy ? "active" : ""}`}>{state.project.template.layoutPolicy ? "已约束" : "未启用"}</span></div><p>生成小角标模板，并锁定贴纸的角落、尺寸和旋转边界。</p><label className="field-label">风格<select value={agentStyle} onChange={(event) => setAgentStyle(event.target.value as LayoutAgentInput["style"])}><option value="black-gold">黑金小角标</option><option value="clean">清爽小角标</option></select></label><label className="field-label">栏目文字<input maxLength={12} value={agentTitle} onChange={(event) => setAgentTitle(event.target.value)} /></label><label className="field-label">价格文字<input maxLength={12} value={agentPrice} onChange={(event) => setAgentPrice(event.target.value)} /></label><div className="agent-rules"><span>贴纸仅四角</span><span>单个 ≤20%宽 / 16%高</span><span>总面积估算 ≤8%</span><span>旋转 ≤15°</span></div><button className="agent-apply" disabled={!agentTitle.trim() || !agentPrice.trim()} onClick={() => onApplyAgent({ style: agentStyle, title: agentTitle, price: agentPrice })}>生成并替换当前模板</button></div><div className="inspector-section"><div className="inspector-title"><span>图层</span><span className="layer-count">{state.project.template.layers.length}/100</span></div><div className="layer-actions"><button onClick={onAddText}>＋文字</button><button onClick={onAddSticker}>＋贴纸</button></div><div className="layer-list">{[...state.project.template.layers].sort((a, b) => b.zIndex - a.zIndex).map((layer) => <LayerRow key={layer.id} layer={layer} selected={layer.id === selectedLayerId} onSelect={() => onSelectLayer(layer.id)} onToggle={() => onPatchLayer(layer.id, { visible: !layer.visible })} onDelete={() => onRemoveLayer(layer.id)} />)}</div>{state.project.template.layers.length === 0 && <div className="empty-inspector">添加文字或贴纸开始编辑。</div>}</div>{selectedLayer && <LayerInspector layer={selectedLayer} onPatch={(patch) => onPatchLayer(selectedLayer.id, patch)} onMove={(direction) => onMoveLayer(selectedLayer.id, direction)} onDelete={() => onRemoveLayer(selectedLayer.id)} />}
    ...
    {!state.templateReadiness.ready && <div className="template-warning" role="alert">模板暂不可导出：{state.templateReadiness.missing.join("、")}。请按提示修复后再导出。</div>}<div className="step-footer template-footer"><div className="safety-note"><span className="preview-symbol">◌</span><span><strong>预览是近似结果</strong><small>导出前生成验证样片，确认最终 FFmpeg 渲染</small></span></div><div className="footer-actions"><button className="secondary-button" disabled={!activeMedia} onClick={onProof}>生成验证样片</button><button className="primary-button" disabled={!state.templateReadiness.ready || state.project.mediaItems.filter((item) => item.probeStatus === "ready").length === 0} onClick={onNext}>去导出 <span>→</span></button></div></div>
  </div>;
}
...
function Preview({ activeMedia, template, selectedLayerId, onSelectLayer, onMoveLayer }: { activeMedia?: MediaView; template: EditTemplate; selectedLayerId?: string; onSelectLayer: (id: string) => void; onMoveLayer: (id: string, patch: Partial<Layer>) => void }) {
  ...
  const dragRef = useRef<{ id: string; session: LayerDragSession }>();
  const onPointerDown = (event: PointerEvent<HTMLDivElement>, layer: Layer) => {
    event.stopPropagation(); onSelectLayer(layer.id); stageRef.current?.setPointerCapture(event.pointerId); dragRef.current = { id: layer.id, session: beginLayerDrag(layer, event.clientX, event.clientY) };
  };
  const onPointerMove = (event: PointerEvent<HTMLDivElement>) => {
    ...
    onMoveLayer(layer.id, cumulativeDragPosition({ ...drag.session, pointerX: event.clientX, pointerY: event.clientY, stageWidth: rect.width, stageHeight: rect.height }));
  };
  const mediaStyle: CSSProperties = { aspectRatio: activeMedia ? `${activeMedia.width} / ${activeMedia.height}` : "16 / 9" };
  return <div ref={stageRef} className="preview-stage" style={mediaStyle} onPointerMove={onPointerMove} onPointerUp={() => { dragRef.current = undefined; }} onPointerCancel={() => { dragRef.current = undefined; }}><div className="preview-grid" />{activeMedia ? <video src={activeMedia.previewUrl} style={{ filter: previewFilter(template.filter) }} muted loop autoPlay playsInline /> : <div className="preview-empty">选择一条有效素材查看预览</div>}{template.layers.filter((layer) => layer.visible).sort((a, b) => a.zIndex - b.zIndex).map((layer) => <PreviewLayer key={layer.id} layer={layer} governed={template.layoutPolicy === CORNER_SAFE_POLICY.id} frameWidth={activeMedia?.width ?? 16} frameHeight={activeMedia?.height ?? 9} selected={layer.id === selectedLayerId} onPointerDown={onPointerDown} />)}<div className="preview-corner-label">PREVIEW / APPROXIMATE</div></div>;
}

function PreviewLayer({ layer, governed, frameWidth, frameHeight, selected, onPointerDown }: { layer: Layer; governed: boolean; frameWidth: number; frameHeight: number; selected: boolean; onPointerDown: (event: PointerEvent<HTMLDivElement>, layer: Layer) => void }) {
  const [sourceSize, setSourceSize] = useState({ width: 1, height: 1 });
  const constrainedSticker = governed && layer.type === "sticker";
  const geometry = constrainedSticker ? constrainedStickerPreviewGeometry({ layer, frameWidth, frameHeight, sourceWidth: sourceSize.width, sourceHeight: sourceSize.height }) : undefined;
  const style: CSSProperties = { left: `${(geometry?.x ?? layer.x) * 100}%`, top: `${(geometry?.y ?? layer.y) * 100}%`, width: `${(geometry?.width ?? layer.width) * 100}%`, height: geometry ? `${geometry.height * 100}%` : undefined, minHeight: constrainedSticker ? 0 : undefined, display: constrainedSticker ? "flex" : undefined, alignItems: constrainedSticker ? "center" : undefined, justifyContent: constrainedSticker ? "center" : undefined, padding: constrainedSticker ? 0 : layer.type === "text" && layer.backgroundColor ? `${(layer.backgroundPaddingRatio ?? 0.006) * 640}px` : undefined, opacity: layer.opacity, transform: constrainedSticker ? undefined : `rotate(${layer.type === "sticker" ? layer.rotationDeg : 0}deg)`, fontFamily: layer.type === "text" ? layer.fontFamily : undefined, fontSize: layer.type === "text" ? `${Math.max(12, layer.fontSizeRatio * 640)}px` : undefined, color: layer.type === "text" ? colorToHex(layer.color) : undefined, WebkitTextStroke: layer.type === "text" ? `${layer.strokeWidthRatio * 640}px ${colorToHex(layer.strokeColor)}` : undefined, background: layer.type === "text" && layer.backgroundColor ? `rgba(${layer.backgroundColor.r}, ${layer.backgroundColor.g}, ${layer.backgroundColor.b}, ${layer.backgroundColor.a})` : undefined };
  const imageStyle: CSSProperties | undefined = geometry ? { width: `${geometry.imageWidth / geometry.width * 100}%`, height: `${geometry.imageHeight / geometry.height * 100}%`, maxWidth: "none", transform: `rotate(${layer.type === "sticker" ? layer.rotationDeg : 0}deg)` } : undefined;
  return <div className={`preview-layer ${layer.type} ${selected ? "selected" : ""}`} style={style} onPointerDown={(event) => onPointerDown(event, layer)}>{layer.type === "text" ? layer.content : <img src={localFileUrl(layer.assetPath)} alt="贴纸图层" draggable={false} style={imageStyle} onLoad={(event) => setSourceSize({ width: event.currentTarget.naturalWidth || 1, height: event.currentTarget.naturalHeight || 1 })} />}</div>;
}
