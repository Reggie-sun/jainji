...
export function CoverTrackEditor({ media, sticker, enabled, staticRectangle, track, disabled, onStaticRectangleChange, onTrackChange }: {
  media: MediaView;
  sticker?: { url: string; label: string };
  enabled: boolean;
  staticRectangle: CoverRectangle;
  track?: CoverTrack;
  disabled: boolean;
  onStaticRectangleChange(rectangle: CoverRectangle): void;
  onTrackChange(track?: CoverTrack): void;
}) {
  ...
  const canEdit = enabled && Boolean(sticker) && !disabled;
  const visible = !track || (timeMs >= track.startMs && timeMs < track.endMs);
  const rectangle = track ? interpolateCoverRectangle(track.keyframes, timeMs) : staticRectangle;
  ...
  const updateStatic = (next: CoverRectangle) => {
    pauseForEdit(); onStaticRectangleChange(next); setNotice("已更新固定覆盖框。");
  };
  ...
  const updateScale = (percent: number) => {
    ...
    const ratio = (track?.keyframes[0]?.rectangle.width ?? staticRectangle.width) / (track?.keyframes[0]?.rectangle.height ?? staticRectangle.height);
  ...
  const updateStaticHeight = (percent: number) => {
    if (!Number.isFinite(percent) || track) return;
  ...
  const moveInteraction = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (!current || !bounds || !canEdit) return;
  ...
  const enableTrack = () => {
    ...
    onTrackChange({ startMs: 0, endMs: durationMs, keyframes: [{ timeMs: 0, rectangle: { ...staticRectangle } }] });
  ...
  const updateInterval = (field: "startMs" | "endMs", value: number) => {
    ...
    onTrackChange({ ...track, startMs, endMs });
  };
  const deleteFrame = (time: number) => {
    if (!track || track.keyframes.length === 1) return;
    onTrackChange({ ...track, keyframes: track.keyframes.filter((frame) => frame.timeMs !== time) });
    setNotice("已删除关键帧。");
  };
  ...
  return <div className="cover-track-editor">
    <div className="cover-track-preview" ref={stage} style={{ aspectRatio: `${media.width} / ${media.height}`, width: `min(100%, ${52 * media.width / media.height}vh)`, marginInline: "auto" }} onPointerMove={moveInteraction} onPointerUp={() => { interaction.current = undefined; }} onPointerCancel={() => { interaction.current = undefined; }}>
      ...
      {enabled && sticker && visible && <div className="cover-sticker-frame" style={{ left: `${rectangle.x * 100}%`, top: `${rectangle.y * 100}%`, width: `${rectangle.width * 100}%`, height: `${rectangle.height * 100}%` }} onPointerDown={(event) => startInteraction(event, "drag")} aria-label="拖动覆盖贴纸"><img src={sticker.url} alt="覆盖贴纸预览" /><div className="cover-sticker-handle" aria-label="等比调整覆盖贴纸尺寸" onPointerDown={(event) => startInteraction(event, "resize")} /></div>}
    </div>
    ...
    {!track ? <div className="cover-track-mode"><div><strong>固定覆盖</strong><p>当前素材沿用所有素材共用的位置和尺寸。启用轨迹后只影响此素材。</p></div><button type="button" className="button secondary compact" disabled={!canEdit} onClick={enableTrack}>为此素材启用轨迹</button></div> : <>
      <div className="cover-track-mode"><div><strong>此素材的关键帧轨迹</strong><p>位置和缩放只影响此素材；拖动、缩放或修改数值会暂停视频并记录当前帧。</p></div><button type="button" className="button secondary compact" disabled={disabled} onClick={() => { onTrackChange(undefined); setNotice("已移除此素材的轨迹，恢复固定覆盖框。"); }}>移除此素材轨迹</button></div>
      ...
      <button type="button" className="button secondary compact" disabled={!canEdit || (track.keyframes.length >= 50 && !track.keyframes.some((frame) => frame.timeMs === timeMs))} onClick={() => upsert(rectangle)}>记录当前关键帧</button>
      {!visible && <p className="cover-sticker-note">当前时间不在出现时段内，覆盖贴纸不会显示；仍可通过时间定位和关键帧列表继续编辑轨迹。</p>}
