...
export function CornerDecorationPicker({ value, onChange, disabled, selected, onSelect }: {
  ...
  return <section id="corner-decoration-editor" aria-label="四角内容设置">
    ...
    {automatic ? <><div className="card corner-settings"><h2>Agent 按画面需要选择</h2><p>根据每条视频选择贴纸和位置。四个角落不必加满，也可以全部留空。</p><p>从可用的内置贴纸与用户上传贴纸中选择，遵守所选模板的尺寸与数量限制。切回“自己设置”可继续使用之前的选择。</p></div><DecorationPicker automatic value={value} disabled={disabled} onChange={onChange} /></> : <>
