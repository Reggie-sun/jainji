...
export class ConnectionStore {
  ...
  get(id: string): { name: string; input: ConnectionInput } {
    ...
    return { name: profile.name, input: ConnectionInputSchema.parse({ baseUrl: profile.baseUrl, model: profile.model, apiKey: profile.apiKey, protocol: profile.protocol, authHeader: profile.authHeader }) };
  }
  ...
  async save(input: unknown): Promise<string> {
    ...
    const profile = { ...fields, apiKey, id: id ?? randomUUID() };
