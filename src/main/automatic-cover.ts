...
export async function recognizeAutomaticCovers(ffmpeg: FfmpegAdapter, media: MediaItem, detect: Detect, signal: AbortSignal): Promise<AutomaticCoverTrack[]> {
  ...
  try {
    ...
    for (let offset = 0; offset < files.length; offset += COVER_DETECTION_WINDOW - 1) {
      ...
      for (let index = offset; index < Math.min(files.length, offset + COVER_DETECTION_WINDOW); index++) {
        ...
        images.push({ timeMs: Math.min(media.durationMs - 1, index * COVER_SAMPLE_INTERVAL_MS), url: `data:image/jpeg;base64,${bytes.toString("base64")}` });
      }
    ...
    return automaticCoverTracks(detected, media.durationMs);
