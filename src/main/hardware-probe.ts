...
export async function probeConcurrentEncodes(singleArgs: string[], count: number, start: StartProbe, timeoutMs = 5_000, threads = 1): Promise<boolean> {
  ...
  args[args.indexOf("-i") + 1] = `${software ? "testsrc2" : "color"}=size=1280x720:rate=30`;
  args.splice(args.indexOf("-f"), 0, "-filter_threads", String(threads), "-threads", String(threads));
  args.splice(args.indexOf("-pix_fmt"), 0, "-threads", String(threads));
